...
def configure_callback(conf):
    ...
    for node in conf.children:
        if node.key == 'Host':
            ...
            collectd.warning('tengine_tenginestats plugin: Unknown config key: %s.'
                            % node.key)
...
def fetch_info():
    url = 'http://%(host)s:%(port)s/tenginestats' % {'host': GATEWAY_HOST, 'port': GATEWAY_PORT}
    ...
    header_to_process = ['kv', 'hostip', 'conn_total', 'req_total', '2xx', '4xx', '5xx', 'rt_total', 'upstream_rt']
    try:
        time1 = time.time()
            ...
            for col in row:
                 if colnum == 0: #VH name
                     ...
                     if colnum == 1:
                         ...
                         if header[colnum] in header_to_process:
...
def log_verbose(msg):
    if not VERBOSE_LOGGING:
        return
    collectd.info('tengine_tenginestats plugin [verbose]: %s' % msg)
